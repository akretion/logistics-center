...
class LogisticsFlow(models.Model):
    ...
    flow = fields.Selection(
        ...
        required="True",
        readonly="1",
        help="Typical logistics flow to implement with an " "external logistics center",
    )
    ...
    def run(self):
        ...
        if self.impacted_record == 0:
            raise UserError(_("There is no data in this flow to convert in file"))
        ...
        body = _("%(name)s %(flow)s has generated " "these pickings : %(records)s") % {
            "name": self.name,
            "flow": self.flow,
            "records": records,
        }
        ...
        kwargs["records"].write({"log_out_file_doc_id": attachment.id})
        ...
        attachment = self.env["ir.attachment"].create(
            {
                ...
                "name": "{}_{}".format(self.flow, kwargs["datas_fname"]),
    ...
    def button_learn(self):
        ...
        return {
            ...
            "res_id": self.env.ref("logistics_center.domain_explain_filter").id,
            "view_mode": "form",
            "target": "current",
        }
    ...
    def _get_records_to_process(self):
        ...
        _logger.info(
            "Flow '%s' retrieve '%s' records with domain %s "
    ...
    def _compute_impacted_records(self):
        for rec in self:
            ...
            rec.impacted_record = self.env["stock.picking"].search_count(domain)
            _logger.debug(
                "Flow '%s' retrieve '%s' records with domain %s "
