...
from .common import DATE_FORMATS, LogisticDialect
...
class Stef(Logistic):
    ...
    def run_flow(self, flow):
        ...
        if flow.direction == "export":
            ...
            if records:
                return self._get_data_to_export(records, flow, type="build_your_own")
            else:
            ...
            domain.extend(
                [
                    ...
                    (
                        "picking_type_id",
                        "=",
                        flow.env.ref("stef_logistics.stef_picking_type").id,
                    ),
                ]
            )
        else:
...
alias produit;type ul;réf lot;qté;poids;date rotation;référence palette client"""
    ...
    def _format_incoming_body(self, mapping):
        return (
            "%(codprd)s;C1;%(num_lot)s;%(qty)s;%(weight)s;%(rotation)s"
            ";;%(palette)s" % mapping
        )
...
alias produit;type ul;réf lot;qté;poids;date rotation;référence palette client"""
    ...
    def _prepare_head_data(self, picking, flow, delivery_head):
        settings = (
            picking.picking_type_id.warehouse_id.logistics_id._logistics_center_settings()
        )
        if flow.logistics_backend_id.version == "stef-portail":
            res = {
                ...
                "trsdst": (
                    ...
                    and picking.company_id.partner_id.stef_partner_id_string
                    or ""
                ),
    ...
    def _export_flow(self, pickings, flow):
        ...
        for picking in pickings:
            ...
            #     vals, head_data, 'head'))
            data.append(getattr(self, "_format_%s_header" % flow.flow)(vals))
            if flow.flow == "delivery":
                for move in picking.move_lines:
                    vals = self._prepare_line_data(move, flow)
                    data.append(getattr(self, "_format_%s_body" % flow.flow)(vals))
            elif flow.flow == "incoming":
