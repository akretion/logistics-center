...
FILE_FLOWS = {
    'PAH': 'logistics_incoming',
    'ODC': 'logistics_delivery',
}
...
class RepositoryTask(models.Model):
    ...
    def move_file(self, connection, task, file_name,
                  folder_path):
        new_filename = '_' + file_name.replace('stef', '')
...
class FileDocument(models.Model):
    ...
    def filter_lines(self, file_doc, raw_lines, startswith=None,
                     context=None):
    ...
    def create(self, vals):
        ...
        return super(FileDocument, self).create(vals)

    def get_product_id(self, product, file_doc, line):
        """ stef require an extension to product (e.g. .ZZ, .OS)
        ...
        return product_m.browse(product_ids[0]).id

    def check_stef_data(
            self, file_doc, data, dtype, line):
        message = (_(" '%s' return by stef file "
        ...
        if dtype == 'product' and not is_numeric(data):
            ...
            raise models.except_models(_("Quantity error"), _("Quantity") + message)
        elif dtype == 'd_order' and not is_numeric(data):
            raise models.except_models(_("Order id error"),
    ...
    def import_delivery(self, file_doc, flow, session):
        struct = {}
        line_number = 0
        for line in self.get_datas_from_file(file_doc, flow['fields'], dialect):
            line_number += 1
            self.check_stef_data(file_doc, line['picking'],
                                      'd_order', line_number)
            picking_id = int(line['picking'])
            self.check_stef_data(file_doc, line['qty'],
                                      'quantity', line_number)
            move = {'product_id': self.get_product_id(
                    line['product_id'], file_doc, line_number),
    ...
    def import_incoming(self, file_doc, flow, session):
        struct = {}
        line_number = 0
        for line in self.get_datas_from_file(
                file_doc, flow['fields'], dialect):
            line_number += 1
            if line['type'] == 'PAL':
                self.check_stef_data(
                    file_doc, line['picking'], 'i_order', line_number)
                picking_id = int(line['picking'])
                self.check_stef_data(
                    file_doc, line['qty_due'], 'quantity', line_number)
                self.check_stef_data(
                    file_doc, line['real_qty'], 'quantity', line_number)
    ...
    def import_datas(self, file_doc, backend):
...
Fill the right type in Task and in File Document and click on run again"""
            ...
            raise models.except_models(_("Error in file document type"),
                                 LOGISTIC_TYPE_ERROR)
        flow = FLOW_PARAMS[file_doc.file_type]
        session = ConnectorSession(context)
        struct = False
        method = 'import_' + file_doc.file_type[9:]
        struct = getattr(self, method)(file_doc, flow, session)
        if struct:
            priority = 100
